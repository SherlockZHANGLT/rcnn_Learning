...
def conv_block(input_tensor, kernel_size, filters, stage, block, train_flag,stride2=False):
    ...
    x = GNReLU(x, name=bn_name_base + '2a')
    ...
    x = GNReLU(x, name=bn_name_base + '2b')
    ...
    shortcut = GNReLU(shortcut, name=bn_name_base + '1')
    x = tf.add(x, shortcut)
...
model_save_path='./save/cifar10/gn'
...
#保存模型
save_path = saver.save(sess,"./save/cifar10/gn/model.ckpt")
print("save model:{0} Finished".format(save_path))
np.save("./save/cifar10/gn/cifar10_step",total_step)
np.save("./save/cifar10/gn/cifar10_loss",total_loss)
np.save("./save/cifar10/gn/cifar10_acc",total_acc)
np.save("./save/cifar10/gn/cifar10_ap",total_ap)
...
np.save("./save/cifar10/gn/cifar10_precision_score",ps)

#召回率
from sklearn.metrics import recall_score
rs=recall_score(y_test, y_log_predict,average=None)
np.save("./save/cifar10/gn/cifar10_recall_score",rs)
