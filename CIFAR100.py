...
for i in range(25000):
    ...
    if i % 100 == 0:
        ...
        for k in range(len(y_log_predict)):
            max_value=max(y_log_predict[k])
            for l in range(len(y_log_predict[k])):
                if max_value==y_log_predict[k][l]:
                    y_log_predict[k][l]=1
                else:
                    y_log_predict[k][l]=0
        ap=average_precision_score(y_train,y_log_predict,average=None)
...
np.save("cifar100_step",total_step)
np.save("cifar100_loss",total_loss)
np.save("cifar100_acc",total_acc)
np.save("cifar100_ap",total_ap)
