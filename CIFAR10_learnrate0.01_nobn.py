...
def conv_block(input_tensor, kernel_size, filters, stage, block, train_flag,stride2=False):
    ...
    x = tf.nn.relu(x, name='relu' + str(stage) + '2c')
    ...
    shortcut=tf.nn.relu(x, name='relu' + str(stage) + 'shortcut')
    x = tf.add(x, shortcut)
...
model_save_path='./save/cifar10/learn_rate0.01_nobn'
data=Cifar10DataReader()
sess=tf.InteractiveSession()

#指数衰减学习率
global_step = tf.Variable(0, trainable=False)
lr = tf.train.exponential_decay(learning_rate=0.01, global_step=global_step, decay_steps=50, decay_rate=0.99, staircase=False)
...
#保存模型
save_path = saver.save(sess,"./save/cifar10/learn_rate0.01_nobn/model.ckpt")
print("save model:{0} Finished".format(save_path))
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_step",total_step)
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_loss",total_loss)
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_acc",total_acc)
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_ap",total_ap)
...
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_precision_score",ps)

#召回率
from sklearn.metrics import recall_score
rs=recall_score(y_test, y_log_predict,average=None)
np.save("./save/cifar10/learn_rate0.01_nobn/cifar10_recall_score",rs)
