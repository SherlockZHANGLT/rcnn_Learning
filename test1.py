...
class hand_classifier(object):
    ...
    def deepnn(self, x_input, classes=100):
        """
        ...
        with tf.variable_scope('reference') :
            ...
            x = tf.nn.avg_pool(x, [1, 1, 1, 1], strides=[1,1,1,1], padding='VALID')
            ...
            logits = tf.layers.dense(x, units=100, activation=tf.nn.softmax)
    ...
    def train(self, X_train, Y_train):
        ...
        labels = tf.placeholder(tf.int64, [None, 100])
    ...
    def evaluate(self, test_features, test_labels, name='test '):
        ...
        y_ = tf.placeholder(tf.int64, [None, 100])
...
def main(_):
    ...
    model.train(x_Train, y_TrainOneHot)
    model.evaluate(x_Test, y_TestOneHot)
    model.evaluate(x_Train, y_TrainOneHot, 'training data')
