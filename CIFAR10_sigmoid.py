...
def conv_block(input_tensor, kernel_size, filters, stage, block, train_flag,stride2=False):
    ...
    x = tf.add(x, shortcut)
    x = tf.nn.sigmoid(x, name='res' + str(stage) + block + '_out')
...
model_save_path='./save/cifar10/sigmoid/'
...
#保存模型
save_path = saver.save(sess,"./save/cifar10/sigmoid/model.ckpt")
print("save model:{0} Finished".format(save_path))
np.save("./save/cifar10/sigmoid/cifar10_step",total_step)
np.save("./save/cifar10/sigmoid/cifar10_loss",total_loss)
np.save("./save/cifar10/sigmoid/cifar10_acc",total_acc)
np.save("./save/cifar10/sigmoid/cifar10_ap",total_ap)
...
np.save("./save/cifar10/sigmoid/cifar10_precision_score",ps)

#召回率
from sklearn.metrics import recall_score
rs=recall_score(y_test, y_log_predict,average=None)
np.save("./save/cifar10/sigmoid/cifar10_recall_score",rs)
